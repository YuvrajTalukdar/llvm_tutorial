...
public:
    ...
    {
        // Print out the statement type for debugging
        /*if (auto *IfStmtNode = dyn_cast<IfStmt>(S)) {
            std::cout << "Found an If Statement!" << std::endl;
        } else if (auto *ForStmtNode = dyn_cast<ForStmt>(S)) {
            std::cout << "Found a For Statement!" << std::endl;
        } else if (auto *ReturnStmtNode = dyn_cast<ReturnStmt>(S)) {
            std::cout << "Found a Return Statement!" << std::endl;
        }*/ 
        if (auto *CallExprNode = dyn_cast<CallExpr>(S)) 
        {
            llvm::outs() << "Found a Function Call: ";
            if(CallExprNode->getDirectCallee())
            {
                llvm::outs() << CallExprNode->getDirectCallee()->getNameInfo().getAsString() << "\n";
                if(!kernel_transformed)
                {
                    if (const FunctionDecl *FD = CallExprNode->getDirectCallee()) 
                    {
                        if (FD->getNameAsString() == "printf") 
                        {
                            ...
                            Rewrite.InsertText(S->getBeginLoc(), "for (int i = 0; i < 10; ++i) {\n\t ", true, true);
                            Rewrite.InsertTextAfterToken(S->getEndLoc(), ";\n\t}");

                            //SourceLocation SemiLoc = S->getExprStmt()->getEndLoc();
